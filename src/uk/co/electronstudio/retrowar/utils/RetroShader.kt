...
class RetroShader(filename: String) {
    ...
    init {
        ...
        if (file.exists()) {
            attemptToLoadShader(file)
        }
    }

    private fun attemptToLoadShader(file: FileHandle?) {
        val potentialShader = ShaderProgram(file, file)
        if (potentialShader.log.isNotEmpty()) {
            log(potentialShader.log)
        }
        if (potentialShader.isCompiled) {
            this.shader = potentialShader
        } else {
            this.shader = null
            uk.co.electronstudio.retrowar.error("failed to compile shader")
        }
    }

    fun process(fboBatch: SpriteBatch, textureSize: Vector2, inputSize: Vector2, outputSize: Vector2) {
        fboBatch.shader = shader
        shader?.let {
            it.setUniformf("TextureSize", textureSize)
            it.setUniformf("InputSize", inputSize)
            it.setUniformf("OutputSize", outputSize)
        }
    }
}
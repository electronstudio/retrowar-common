...
open class Menu(
    ...
    internal var selectedItem =
        0

    var editing: MenuItem? =
        null
    internal val indices: IntRange
        get() = currentPage.indices
    val selectedItemIndex: Int
        get() = allItems.indexOf(
    ...
    fun addAll(
        elements: Collection<MenuItem>
    ): Boolean {
        return allItems.addAll(
            elements
        )
    }
    ...
    fun up() {
        ...
        while (selectedItem > 0 && currentPage.get(
                selectedItem
            ).isHidden
        ) {
            selectedItem--
    ...
    fun down() {
        ...
        while (selectedItem < currentPage.size - 1 && currentPage.get(
                selectedItem
            ).isHidden
        ) {
            selectedItem++

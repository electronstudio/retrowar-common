...
import uk.co.electronstudio.retrowar.createDefaultShapeShader
import uk.co.electronstudio.retrowar.drawBox
...
class MenuController(
    ...
    //  input.isKeyJustPressed(RIGHT) ||
        input.isKeyJustPressed(SPACE) || input.isKeyJustPressed(ENTER) || app.mouseJustClicked || app.statefulControllers.any { it.isButtonAJustPressed }

    private fun inputRight() =
        input.isKeyJustPressed(RIGHT) || app.statefulControllers.any { it.isRightButtonJustPressed }

    private fun inputLeft() = input.isKeyJustPressed(LEFT) || app.statefulControllers.any { it.isLeftButtonJustPressed }

    private fun inputDown() = input.isKeyJustPressed(DOWN) || app.statefulControllers.any { it.isDownButtonJustPressed }

    private fun inputUp() = input.isKeyJustPressed(UP) || app.statefulControllers.any { it.isUpButtonJustPressed }
}
...
//        batch.begin()
    ...
    fun doInput() {
        ...
        if (menu.editing is StringPrefMenuItem) {
            doTextInput(
                menu.editing as StringPrefMenuItem
            )
            return
        }
    ...
    private fun doTextInput(
        item: StringPrefMenuItem
    ) {
    ...
    val lineHeightPixels: Float =
        -font.data.down

    @Suppress(
        "UNUSED_PARAMETER"
    )
    private fun mouseSelect(
        menu: Menu,
        mx: Float,
        my: Float
    ) {
        ...
        for (i in menu.indices) {
            ...
            val d =
                if (menu.doubleSpaced) 2 else 2
            val top =
                y - d * lineHeightPixels - counter - lineHeightPixels / 2

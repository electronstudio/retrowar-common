...
class ManagedFBO {
    val MAX_WIDTH =
        Gdx.graphics.displayMode.width
    val MAX_HEIGHT =
        Gdx.graphics.displayMode.height
    ...
    internal var fboCam: OrthographicCamera =
        OrthographicCamera()
    ...
    fun resize(
        ...
        val camWidth =
            MAX_WIDTH * scale // renderer.WIDTH
        val camHeight =
            MAX_HEIGHT * scale // renderer.HEIGHT
        fboCam.setToOrtho(
            false,
            camWidth,
            camHeight
    ...
    fun resizeToScreenSize(
        ...
        val camWidth =
            MAX_WIDTH.toFloat() / (width / w)
        val camHeight =
            MAX_HEIGHT.toFloat() / (height / h)
    ...
    fun updateShader(
        ...
        val textureSize =
            Vector2(
                Gdx.graphics.displayMode.width.toFloat(),
                Gdx.graphics.displayMode.height.toFloat()
            )
        app.shader.process(
            fboBatch,
            textureSize,

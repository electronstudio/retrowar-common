...
class FBORenderer(
    ...
    internal var scaleFactor =
        1f

    var timer =
        0.0f

    internal var fboBatch =
        SpriteBatch(
            100,
            createDefaultShader()
        )
    ...
    private fun endFBO() {
        batch.begin()
        if (FPS.isEnabled()) {
            Resources.FONT_ENGLISH.setColor(
                Color.WHITE
            )
            glyphLayout.setText(
                Resources.FONT_ENGLISH,
                "FPS ${Gdx.graphics.framesPerSecond}"
            )
            Resources.FONT_ENGLISH.draw(
                batch,
                glyphLayout,
    ...
    fun findAppropriateScaleFactor(
        w: Float,
        h: Float
    ): Float =
        if (Prefs.BinPref.STRETCH.isEnabled()) findHighestScaleFactor(
            w,
            h
        )
        else findHighestIntegerScaleFactor(
            w,
            h
        )

    fun findHighestIntegerScaleFactor(
        width: Float,
        height: Float
    ): Float {
        val w =
    ...
    fun convertGameCoordsToScreenCoords(
        x: Float,
        y: Float
    ): Vector3 {
        return cam.project(
